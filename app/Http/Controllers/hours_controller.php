<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Models\{
    hours
};
use DataTables;
class hours_controller extends Controller
{
    public function index(){
        $data = [
            'menu' => 'master3',
            'submenu' => 'hours',
        ];

        return view('user.admin.hours.index',$data);
    }
    public function get_data(Request $request){
        if ($request->ajax()) {
            $data = hours::latest()->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function($row){

                    if($row->is_deleted == 'Y'){
                        $actionBtn = '  <button class="btn btn-sm btn-warning edit" data-bs-toggle="modal" data-bs-target="#edit_modal" value="'.$row->id.'">
                                            <i class="fa fa-edit"></i>
                                        </button>
                                        <button class="btn btn-sm btn-danger hapus" value="'.$row->id.'" disabled>
                                            <i class="fa fa-trash"></i>
                                        </button>';
                    }else{
                        $actionBtn = '  <button class="btn btn-sm btn-warning edit" data-bs-toggle="modal" data-bs-target="#edit_modal" value="'.$row->id.'">
                                            <i class="fa fa-edit"></i>
                                        </button>
                                        <button class="btn btn-sm btn-danger hapus" value="'.$row->id.'">
                                            <i class="fa fa-trash"></i>
                                        </button>';
                    }
                    return $actionBtn;
                })
                ->rawColumns(['action'])
                ->make(true);
        }
    }
    public function store(Request $request){
        $hours = new hours();
        $hours->name = $request->name;
        $hours->start = $request->start;
        $hours->end = $request->end;
        $hours->save();

        return response()->json(
            [
                'msg' => 'Success',
            ],
            200
        );
    }
    public function show($id){
        return response()->json(
            [
                'data' => hours::find($id),
            ],
            200
        );
    }
    public function update(Request $request,$id){
        $hours = hours::find($id);
        $hours->name = $request->name;
        $hours->start = $request->start;
        $hours->end = $request->end;
        $hours->save();

        return response()->json(
            [
                'msg' => 'Success',
            ],
            200
        );
    }
    public function destroy($id){
        $hours = hours::find($id);
        $hours->delete();
        return response()->json(
            [
                'msg' => 'Success',
            ],
            200
        );
    }
}
