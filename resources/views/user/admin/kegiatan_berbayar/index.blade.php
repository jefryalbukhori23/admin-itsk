    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Form Tambah Kegiatan Berbayar</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    <label for="recipient-name" class="form-label">Nama Kegiatan Berbayar</label>
                    <input type="text" class="form-control" id="name" placeholder="Masukan Nama Kegiatan Berbayar">
                </div>
            </div>
...
<div id="edit_modal" class="modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Form Edit Kegiatan Berbayar</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="name_edit" placeholder="Masukan Nama Kegiatan Berbayar" readonly>
                </div>
            </div>
            <div class="modal-footer">
...
<script>
    $(document).ready(function(){
        var table = $('#tabel').DataTable({
            ...
            ajax: "{{ url('get-kegiatan_berbayar') }}",
            columns: [
        ...
        $("body").on('click','.save_data',function(){
            ...
            if(name == ""){
                ...
                var url_save = '{{route('kegiatan_berbayar.store')}}';
                $.ajax({
                    type : 'POST',
                    url : url_save,
                    data : {
                    ...
                    success:function(response){
                        ...
                        swal.fire({
                            icon: 'success',
                            title: 'Data Kegiatan Berbayar berhasil ditambahkan !',
                            timer:1500,
                        });
        ...
        $("body").on('click','.edit',function(){
            var id = $(this).val();
            var url_detail = "{{ route('kegiatan_berbayar.show', ':id') }}";
        ...
        $("body").on('click','.update',function(){
            // alert('tes');
            var id = $("#id_edit").val();
            var url_update = "{{ route('kegiatan_berbayar.update', ':id') }}";
            ...
            $.ajax({
                ...
                success:function(response){
                    $("#name_edit").val('');
                    // get_kegiatan_berbayar();
                    swal.fire({
                        icon: 'success',
                            title: 'Data Kegiatan Berbayar berhasil diupdate !',
                            timer:1500,
                    });
                    $('#tabel').DataTable().ajax.reload();
        ...
        $("body").on('click','.hapus',function(){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var id = $(this).val();
                    var url_delete = "{{ route('kegiatan_berbayar.destroy', ':id') }}";
